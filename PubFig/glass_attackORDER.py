...
def get_md(model, X, t): 
    with torch.no_grad():
        ...
        loss = md_loss_targeted(outputs, t)
...
if __name__ == "__main__":
    ...
    alpha   = 20
    itera   = [1]#[1 , 2  , 3  , 5  , 7  , 10 , 20 , 50 , 100 , 300 ] # this is default numbers we used in experiment
    restart = 1
    Nclass=60
    CUT=0
    ...
    for i in range(len(itera)):
        ...
        record_prev=np.zeros((Nclass-CUT,total))
        record_after=np.zeros((Nclass-CUT,total))
        ...
        for k in range(Nclass-CUT):
            ...
            for j in range(int(np.ceil(total/batch_size))):
                ...
                record_prev[k][start:end]=get_md(model, images, targets)
                X_adv=glass_attack(model, images, targets,glass, alpha ,itera[i])
                record_after[k][start:end]=get_md(model, X_adv, targets)
            with open('ORDERprev','wb') as f:
                pickle.dump(record_prev,f)
            with open('ORDERafter','wb') as f:
                pickle.dump(record_after,f)
