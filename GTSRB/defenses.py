...
def GroupAdvTrain_epoch2(model,optim, x,y,batch_size,iters,eps=16/255,m=8,device='cpu',dataset="mnist",L_dist='L_inf',v=0.1):
    model.train()
    model.to(device)
...
def FreeAdvTrain(model,optim, trainx,trainy,testx,testy,batch_size,max_iters=5,eps=16/255,m=8,device='cpu',dataset="mnist",L_dist='L_inf',nclass=10,x_val=None,y_val=None,origins=None,pairs=None):
    ...
    for i in range(max_iters):
        ...
        model.eval()
        test(model,testx,testy,512,device='cpu')
        advx=pgd_attack(model, testx,testy,batch_size=1024,eps=8/255, alpha=8/255, iters=1)
        test(model,advx,testy,512,device='cpu')
        """
...
def GroupTrain(model,optim, trainx,trainy,testx,testy,batch_size,max_iters=5,eps=16/255,m=8,device='cpu',dataset="mnist",L_dist='L_inf',nclass=10,x_val=None,y_val=None,origins=None,pairs=None,v=0.1):
    ...
    for i in range(max_iters):
        model,optim,loss[i][0],loss[i][1]=GroupAdvTrain_epoch2(model,optim, x,y,batch_size,i,eps=eps,m=m,device=device,dataset=dataset,L_dist=L_dist,v=v)
        model.eval()
        """
