...
class AutoAttack():
    ...
    def run_standard_evaluation(self, x_orig, y_orig, bs=250, return_labels=False,glass=None):
        if self.verbose:
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(n_batches):
                ...
                correct_batch = y.eq(y)#y.eq(output)
            ...
            for attack in self.attacks_to_run:
                ...
                for batch_idx in range(n_batches):
                    ...
                    if attack == 'apgd-ce':
                        ...
                        self.apgd.loss = 'dlr'
                        self.apgd.seed = self.get_seed()
                        adv_curr = self.apgd.perturb(x, y,glass=glass) #cheap=True
