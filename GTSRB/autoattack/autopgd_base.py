...
class APGDAttack():
    ...
    def normalize(self, x):
        if self.norm == 'Linf':
            ...
            #return x / (t.view(-1, *([1] * self.ndims)) + 1e-12)
            t = (x ** 2).contiguous().view(x.shape[0], -1).sum(-1).sqrt()
            return x / (t.contiguous().view(-1, *([1] * self.ndims)) + 1e-12)
    ...
    def lp_norm(self, x):
        if self.norm == 'L2':
            t = (x ** 2).contiguous().view(x.shape[0], -1).sum(-1).sqrt()
            return t.contiguous().view(-1, *([1] * self.ndims))
    ...
    def attack_single_run(self, x, y, x_init=None,glass=None):
        ...
        for i in range(self.n_iter):
            ...
            with torch.no_grad():
                ...
                if glass is None:
                    ...
                    x_adv=(x_adv_1-x_adv)*glass+x_adv
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None,glass=None):
        """
            ...
            for counter in range(self.n_restarts):
                ...
                if ind_to_fool.numel() != 0:
                    ...
                    if not self.use_largereps:
                        res_curr = self.attack_single_run(x_to_fool, y_to_fool,glass=glass)
                    else:
