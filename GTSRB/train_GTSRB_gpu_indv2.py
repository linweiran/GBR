...
for random_seed in range(100):
    ...
    #model=NormalTrain(ADVnetwork,ADVoptimizer, x_train, y_train,x_test,y_test,batch_size,max_iters=100,eps=eps,m=8,device=device,dataset='GTSRB',nclass=nclass,x_val=x_val,y_val=y_val,origins=origins,pairs=pairs)
    ...
    acclT.append(test(model,x_t,y_t,batch_size,device=device))
    with open('models/GTSRB-performance3-L_inf-indivlgacclT','wb') as f: 
        pickle.dump(acclT,f)
    """
