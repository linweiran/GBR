...
if distance=="linf":
    ...
    with open("indiceslinfM",'rb') as f:
...
for tag in indices:
    ...
    for images in indice:
        ...
        while (len(hits)<g):
            ...
            row=idx // i
            col=idx % i
            if row >= ordering.shape[0]//i-5:
                Mflag=True
            if (Mflag==False) and (row<(ordering.shape[0]//i)-5) and (len(hits)==g-1):
                continue
            if row in hits:
                continue
        ...
        counts.append(total)
    record[tag]=counts
with open("TIME-"+distance+"-"+method+"M",'wb') as f:
