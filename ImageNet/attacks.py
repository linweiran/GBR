...
class PGD():
        ...
        for _ in range(self.eot_iter):
            with torch.enable_grad():
                ...
                if self.loss=='md_single' or self.loss=='ce':
                    ...
                    loss_indiv = lo(logits,y_targets)
                loss = loss_indiv.sum()
                print (loss_indiv.detach().cpu().numpy())
    ...
    def get_md(self, x_in, y_in,y_targets,bs=512):
        ...
        before=np.zeros(x.shape[0])
        after=np.zeros(x.shape[0])
        for batch_idx in range(n_batches):
                ...
                logits = self.model(x_to_fool)
                raw=self.md_loss_single(logits,y_t).detach().cpu().numpy()
                adv_samples,tmp=self.attack_single_run_auto(x_to_fool, y_to_fool,y_targets=y_t,y_t=y_t)
                
                logits = self.model(tmp)
                baked=self.md_loss_single(logits,y_t).detach().cpu().numpy()
                print ('{',raw,baked,'}')
                before[start_idx:end_idx]=raw
                after[start_idx:end_idx]=baked
        return before,after
