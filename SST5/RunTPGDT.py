...
parser.add_argument('--target_class', type=int, default=4,help="target class, a integer in [0,4]")
...
from utils import Block, MakeDataset, Prober, Attacker,ProberT,AttackerT
...
probert = ProberT.ProberT(tokenizer, probe_layer, victim_model, target_class=target_class, victim_tokenizer = victim_tokenizer, victim_cuda_device=victim_device, model_cuda_device=cuda_device, decode_mode = decode_mode)

attackert = AttackerT.PandRAttacker(model, dataset, encoded_dataset, text_key, prober,probert,target_class=target_class, model_cuda_device=cuda_device, stop_ids = stop_ids, target_metric = args.target_metric)
...
for index in tqdm(range(args.start, args.end)):
    t=time.time()
    result = attackert.attack_step(index = index, bs_lower_limit = args.bs_lower_limit, bs_upper_limit = args.bs_upper_limit, task_loss_weight = 1.0, decode_loss_weight = args.decode_weight,  init_mag = args.init_mag,
...
"""
with open(str(args.target_class)+"Targeted"+str(args.start)+"-"+str( args.end),'wb') as f:
    pickle.dump((succ,uses,ts),f)
