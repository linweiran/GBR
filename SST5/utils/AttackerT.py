...
class PandRAttacker:
    ...
    def __init__(self, model, dataset, encoded_dataset, text_key, prober,proberT,target_class, model_cuda_device, stop_ids, target_metric = 'bs'):
    ...
    def model_forward(self, input_ids, attention_mask, labels, decode_hidden_index = -1):
        ...
        loss=100-torch.sum(torch.nn.functional.relu(logits+1e-15-logits[0][self.target_class]))
    ...
    def attack_step(self, index = 0, bs_lower_limit = 0.0, bs_upper_limit = 0.50, task_loss_weight = 1.0, decode_loss_weight = 1.0,  init_mag = 1.0,
                ...
                if not id.tolist() in id_base:
                    ...
                    if not result.flipped :
                        ...
                        if result.bs > outputs['max_bs']:
                            result.succeed, victim_loss = self.proberT.attack(result.adv_sentence, label, _ori_sentence = _ori_sentence, task = task, print_info = attack_info )
                            result.query += 1
        ...
        loss, reconstruct_ids = self.proberT.decode(origin_hidden, origin_ids = ori_input_ids, decode_mask = decode_mask, perturb_positions = perturb_positions, stop_ids = self.stop_ids)
        ...
        decode_loss, init_ids = self.proberT.decode(hidden, origin_ids = input_ids, decode_mask = decode_mask, perturb_positions = perturb_positions, stop_ids = self.stop_ids)
        ...
        for seg_step in range(num_seg_steps):
            ...
            for adv_step in adv_accu_loop:
                ...
                torch.manual_seed(SEED)
                loss, logits, hidden =  self.model_forward(input_ids=input_ids, attention_mask=attention_mask, labels = label, decode_hidden_index = decode_hidden_index)
                decode_loss, p_ids = self.proberT.decode(hidden, origin_ids = input_ids, decode_mask = decode_mask, perturb_positions = perturb_positions, stop_ids = self.stop_ids)
