...
class PandRAttacker:
    ...
    def __init__(self, model, dataset, encoded_dataset, text_key, prober,proberMAX, model_cuda_device, stop_ids, target_metric = 'bs'):
    ...
    def model_forward(self, input_ids, attention_mask, labels, target_classes,decode_hidden_index = -1):
        ...
        logitsC=logits[0]
        logitsT=logitsC[target_classes]
        logitsTMAX=logitsT.max()
        loss=100-torch.sum(torch.nn.functional.relu(logitsC+1e-15-logitsTMAX))
    ...
    def attack_step(self, target_classes,index = 0, bs_lower_limit = 0.0, bs_upper_limit = 0.50, task_loss_weight = 1.0, decode_loss_weight = 1.0,  init_mag = 1.0,
                adv_lr = 3e-2, num_seg_steps = 5, num_adv_steps = 3, perturb_layer_index = 0, decode_hidden_index = -1, average_input_length = 20,
                use_random_cover = True, show_info = True, task = None, attack_info = False, 
                ...
                if not id.tolist() in id_base:
                    ...
                    if not result.flipped :
                        ...
                        if result.bs > outputs['max_bs']:
                            result.succeed, victim_loss = self.proberMAX.attack(result.adv_sentence, label,target_classes=target_classes, _ori_sentence = _ori_sentence, task = task, print_info = attack_info )
                            result.query += 1
        ...
        origin_loss, origin_logits, origin_hidden = self.model_forward(input_ids=input_ids, attention_mask=attention_mask, labels = label, target_classes=target_classes,decode_hidden_index = decode_hidden_index)
        loss, reconstruct_ids = self.proberMAX.decode(origin_hidden, origin_ids = ori_input_ids, decode_mask = decode_mask, perturb_positions = perturb_positions, stop_ids = self.stop_ids)
        ...
        torch.manual_seed(SEED)
        loss, logits, hidden = self.model_forward(input_ids=input_ids, attention_mask=attention_mask, labels = label, target_classes=target_classes,decode_hidden_index=decode_hidden_index)
        decode_loss, init_ids = self.proberMAX.decode(hidden, origin_ids = input_ids, decode_mask = decode_mask, perturb_positions = perturb_positions, stop_ids = self.stop_ids)
        ...
        for seg_step in range(num_seg_steps):
            ...
            for adv_step in adv_accu_loop:
                ...
                torch.manual_seed(SEED)
                loss, logits, hidden =  self.model_forward(input_ids=input_ids, attention_mask=attention_mask, labels = label,target_classes=target_classes, decode_hidden_index = decode_hidden_index)
                decode_loss, p_ids = self.proberMAX.decode(hidden, origin_ids = input_ids, decode_mask = decode_mask, perturb_positions = perturb_positions, stop_ids = self.stop_ids)
