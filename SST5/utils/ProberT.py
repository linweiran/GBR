...
class ProberT():
    ...
    def __init__(self, tokenizer,  probe_layer, victim_model, target_class,victim_cuda_device = 0, model_cuda_device = 0, decode_mode = 'Bert', victim_tokenizer = None):
    ...
    def attack(self, reconstruction_sentence, answer_label, print_info = False, _ori_sentence = None, task = None):
        ...
        """
        return (True, loss) if adv_answer.equal(torch.tensor([self.target_class]).cuda(self.model_cuda_device)) else (False, loss)
