...
import argparse
import time
#target_dict={0:[2,3,4],1:[2,3,4],2:[0,1,3,4],3:[0,1,2],4:[0,1,2]}#A
#target_dict={0:[1,2,3,4],1:[2,3,4],2:[3,4],3:[4],4:[4]}#B
#target_dict={0:[0],1:[0],2:[0,1],3:[0,1,2],4:[0,1,2,3]}#C
...
if args.impersonation=="A":
    target_dict={0:[2,3,4],1:[2,3,4],2:[0,1,3,4],3:[0,1,2],4:[0,1,2]}#class 2 to be filtered out in results according to labels
if args.impersonation=="B":
    target_dict={0:[1,2,3,4],1:[2,3,4],2:[3,4],3:[4],4:[4]}#class 4 to be filtered out in results according to labels
if args.impersonation=="C":
    target_dict={0:[0],1:[0],2:[0,1],3:[0,1,2],4:[0,1,2,3]}#class 0 to be filtered out in results according to labels
...
from utils import Block, MakeDataset, Prober, Attacker,ProberMAX,AttackerMAX
...
print (dataset, encoded_dataset, num_labels, text_key, stop_ids)
#use test set
dataset=dataset['test']
encoded_dataset=encoded_dataset['test']
...
probermax = ProberMAX.ProberMAX(tokenizer, probe_layer, victim_model, victim_tokenizer = victim_tokenizer, victim_cuda_device=victim_device, model_cuda_device=cuda_device, decode_mode = decode_mode)

attackermax = AttackerMAX.PandRAttacker(model, dataset, encoded_dataset, text_key, prober,probermax, model_cuda_device=cuda_device, stop_ids = stop_ids, target_metric = args.target_metric)
...
for index in tqdm(range(args.start, args.end)):
    tc=target_dict[dataset['label'][index]]
    t=time.time()
    result = attackermax.attack_step(target_classes=tc,index = index, bs_lower_limit = args.bs_lower_limit, bs_upper_limit = args.bs_upper_limit, task_loss_weight = 1.0, decode_loss_weight = args.decode_weight,  init_mag = args.init_mag,
...
for r in results:
    ...
    if r is not None:
        ts.append(times[num_predict_failed+num_predict_succeed])
        ...
        if r['max_bs'] <= args.eval_lower_limit:
            ...
            uses.append(0)
            bss.append(0)
            labbels.append(dataset['label'][count])
        else:
...
"""
with open("MAXB"+str(args.start)+"-"+str( args.end),'wb') as f:
    pickle.dump((succ,uses,labbels,ts),f)
